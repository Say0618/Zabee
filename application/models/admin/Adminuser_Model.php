...
class Adminuser_Model extends CI_Model
{
	...
	function getbackendUsers2($search, $offset, $length)
	{
		$this->datatables->select('bu.admin_id as id, admin_username, admin_password, user_type, admin_name, admin_email, admin_mobile')			
						 ->from('tbl_backend_users as bu')
						 ->where('bu.admin_id !=', 0);
		$this->db->like('admin_username', $search);
		$this->db->order_by('bu.admin_id', 'asc');	
