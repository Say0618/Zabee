...
$.fn.dataTable.pipeline = function ( opts ) {
    ...
    return function ( request, drawCallback, settings ) {
        ...
        if ( ajax ) {
			...
			request.is_active = "<?php echo $_GET['v']?>";
		...
		"columnDefs": [
			...
			<?php }?>
			{
                "render": function ( data, type, row ) {
					...
					var path ="";
					if(row.is_local == '1'){
						img = '<img src="<?php echo product_thumb_path()?>'+row.is_primary_image+'" class="img-fluid" style="max-height:100px;display: block;margin-left: auto;margin-right: auto;">';
					}else if(row.is_local =="0"){
						img = '<img src="'+row.is_primary_image+'" class="img-fluid" style="max-height:100px;display: block;margin-left: auto;margin-right: auto;">';	

					}else{
						img = '<img src="<?php echo product_thumb_path()?>Preview.png" class="img-fluid" style="max-height:100px;display: block;margin-left: auto;margin-right: auto;">';	
					}
					return img;
                },
			...
			{
                "render": function ( data, type, row ) {
					var enable = '<button class="btn btn-enable" data-prodid="'+row.product_id+'" data-isactive="'+row.active+'" data-value = "1" onclick="toggleactive(this)">Enable</button>';
					var disable = '<button class="btn btn-disable" data-prodid="'+row.product_id+'" data-isactive="'+row.active+'" data-value = "0" onclick="toggleactive(this)">Disable</button>';
					var disableBtn = (row.active == 1)?enable:disable;
					var dropdown_button = '<div class="dropdown dropdown-padding"><button class="btn btn-secondary smallButton" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="fa fa-cog fa-3" aria-hidden="true"></i></button><div class="dropdown-menu" aria-labelledby="dropdownMenuButton"><a class="dropdown-item" href="<?php echo base_url('seller/product/create?pi='); ?>'+row.product_id+'"><i class="fa fa-edit"></i>     Edit</a></div>';
					//DELETE BUTTON (APPEND ABOVE)
					//<div class="dropdown-divider"></div><button class="btn btn-outline" style="color: #212529!important;font-weight: 400!important;" data-prodid="'+row.product_id+'" id="delete" onclick="deletestatus(this)"><i class="fa fa-trash"></i>     Delete</button>
			...
			{
                "render": function ( data, type, row ) {
					var approve_button = '<div class="text-center"><a class="btn btn-success"  data-productid="'+row.product_id+'" data-sellerid="'+row.userid1+'" onclick="askApprove(this)" style="color:white; width:95px;">Approve</a></div>';
					var decline_button = '<div class="text-center"><a class="btn btn-danger"  data-productid="'+row.product_id+'" data-sellerid="'+row.userid1+'" onclick="askDecline(this)" style="color:white; width:95px;margin-top: 10px;">Decline</a></div>';
...
function askApprove(identifier){
	...
	$("#p_id").val($(identifier).data('productid'));
...
$('#rejectBtn').on('click',function(){
	...
	if(reason == ""){
		...
		$.ajax({
			...
			dataType: "JSON",
			async:true,
			url: "<?php echo base_url('seller/product/declineProduct');?>",
			data: {'productid':productid,'sellerid' :sellerid,'reason':reason},
			success: function(response){
				if(response.success == true){
					setTimeout(function(){
						location.reload();
