  ...
  // var end;
  	 ...
  	 t = $('#datatables').DataTable({
		...
		"ajax": $.fn.dataTable.pipeline( {
            url: '<?php echo base_url('warehouse/dashboard/get_product_details')?>',
            pages: 5 // number of pages to cache
		}),
		"columnDefs": [
            {
            ...
            },
			{
                "render": function ( data, type, row ) {
					...
					var disableTooltip = (row.is_active == 1)?enable:disable;
			...
			},
			{
                ...
                "render": function ( data, type, row ) {
                    var button = '<a href="<?php echo base_url(); ?>product/detail/'+btoa(encodeURIComponent(row.product_name+"_"+row.product_id))+'" class="btn btn-info"><?php echo $this->lang->line('view');?></a>';
                    return button;
                },
