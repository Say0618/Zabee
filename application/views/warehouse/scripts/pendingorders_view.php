   ...
   var end;
  	t = $('.datatables').DataTable({
		...
		"ajax": $.fn.dataTable.pipeline( {
            url: '<?php echo base_url('warehouse/sales/pendingorders_data')?>',
            pages: 5 ,// number of pages to cache
            method: "POST"
		} ),
        ...
        if (row.child.isShown()) {
            ...
            //if(isClick == 0){
                $.ajax({
                    ...
                    data:{'o_id':order_id, 'data_from':'pending'},
...
$(document).on('click', '#rejectBtn', function(){
	...
	if(reason == ""){
		...
		$.ajax({
			...
			data: {'order_id':trans_Id, 'trans_details_Id':trans_details_Id,'sellerid' :sellerid},
			success: function(response){
				if(response){
